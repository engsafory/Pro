...
add_action( 'rest_api_init', function () {
  register_rest_route( 'Wp/v2', '/entity', array(
    'methods' => 'GET',
    'callback' => 'handle_getEntity_all',
    'permission_callback' => function () {
      return current_user_can( 'edit_others_posts' );
    }
  ) );
...
function handle_getEntity_all( $data ) {
    global $wpdb;
	//print_r($data); 
	$jsonArray = $data->get_get_params();
		// echo $jsonArray['safory'];
		//  echo $jsonArray['love'];
		//   echo $jsonArray['wife'];
	 $where =" Where 1=1 " ;
	 if($jsonArray['name'] <> null )
	 {$where = $where." and name like'%".$jsonArray['name']."%'"; }
 	 if($jsonArray['loc'] <> null )
	 {$where =$where." and ( cell1 ='".$jsonArray['cell1']."'" ;}
 	 if($jsonArray['loc'] <> null )
	 {$where =$where." or cell2 ='".$jsonArray['cell2']."'" ;}
 	 if($jsonArray['loc'] <> null )
	 {$where =$where." or cell3 ='".$jsonArray['cell3']."'" ;}
 	 if($jsonArray['loc'] <> null )
	 {$where =$where." or cell4 ='".$jsonArray['cell4']."'" ;}
 	 if($jsonArray['loc'] <> null )
	 {$where =$where." or cell5 ='".$jsonArray['cell5']."'"; } 
  if($jsonArray['city_ar'] <> null )
	 {$where =$where." and  city_ar ='".$jsonArray['city_ar']."'"; }
  if($jsonArray['state_ar'] <> null )
	 {$where =$where." and state_ar ='".$jsonArray['state_ar']."'"; }
   if($process_date['date'] == $process_date['date']  )
	 { $query = "SELECT   place_id, name, formatted_address, adr_address, formatted_phone_number, rating, geometry_location_lat, geometry_location_lng, place, Search  FROM `Entity` ".$where." LIMIT 50";
   ///echo $query;
   //echo $query;
    $list = $wpdb->get_results($query);
    return $list; }
...
function process_date( $data ) {
   
    return ((($data+33232434)*23)+333443)*11;
}
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'Wp/v2', '/date', array(
    'methods' => 'GET',
    'callback' => 'handle_getdate',
    'permission_callback' => function () {
      return current_user_can( 'edit_others_posts' );
    }
  ) );
...
function handle_getdate( $data ) {
   ...
   //echo $query;
	...
	if($jsonArray['param1'] <> null && $jsonArray['param2'] <> null )
	{
		if ( ( date('Ymdhis', time())- $jsonArray['param1'])<10000000000)
		{  
			if(process_date( $jsonArray['param1']  )==$jsonArray['param2'])
			{return "good";}
		else { return process_date( $jsonArray['param1'] );}
			
			}
else {return  date('Ymdhis', time());}
	}
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'Wp/v2', '/hos', array(
    'methods' => 'GET',
    'callback' => 'handle_gethos_all',
    'permission_callback' => function () {
      return current_user_can( 'edit_others_posts' );
    }
  ) );
...
function handle_gethos_all( $data ) {
    global $wpdb;
	//print_r($data); 
	$jsonArray = $data->get_get_params();
		// echo $jsonArray['safory'];
	 ...
	 if($jsonArray['name'] <> null )
	 {$where = $where." and name like'%".$jsonArray['name']."%'"; }
 	 if($jsonArray['cell1'] <> null )
	 {$where =$where." and cell1 ='".$jsonArray['cell1']."'" ;}
 	 if($jsonArray['cell2'] <> null )
	 {$where =$where." and cell2 ='".$jsonArray['cell2']."'" ;}
 	 if($jsonArray['cell3'] <> null )
	 {$where =$where." and cell3 ='".$jsonArray['cell3']."'" ;}
 	 if($jsonArray['cell4'] <> null )
	 {$where =$where." and cell4 ='".$jsonArray['cell4']."'" ;}
 	 if($jsonArray['cell5'] <> null )
	 {$where =$where." and cell5 ='".$jsonArray['cell5']."'"; } 
 
 
 
	
	 
    $query = "SELECT  * FROM `hosp` ".$where." LIMIT 50";
   ///echo $query;
   ///echo $query;
    $list = $wpdb->get_results($query);
    return $list;
...
function handle_POhos_all( $data ) {
    global $wpdb;
	 $jsonArray = $data->get_json_params();
	 foreach($jsonArray as $result) {
    //echo $result[name].'<br>';
	$query = "INSERT INTO  `hosp` ( NAME ) VALUES ('".$result[name]."--safory')";
    $list = $wpdb->get_results($query);
}
    return $list;
}
...
function handle_POSThos_all( $data ) {
    global $wpdb;
	 $jsonArray = $data->get_json_params();
	 foreach($jsonArray as $result) {
    //echo $result[name].'<br>';
	$query = "UPDATE `hosp` set NAME  = '".$result[name]."-safory' where place_id ='".$result[place_id]."'";
    $list = $wpdb->get_results($query);
}
    return $list;
}